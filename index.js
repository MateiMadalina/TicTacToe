...
//if the function getUnbeatableAiCoordinates is used, then currentPlayer should take the position indicated by easyLose (variable initialized with the function)
if(easyLose){
    board[easyLose[0]][easyLose[1]] = currentPlayer;
    gameTurn += 1;
    displayBoard(board);
}
//If easyLose is not called, then 'ai' enters the loop of easyWin, in which certain moves are set so that he cannot be defeated
else{
let easyWin = getUnbeatableAiCoordinates(board, currentPlayer);
//console.log(easyWin)
//on the if loop if 'ai' sees an opportunity to win, his move will be put in that position
if(easyWin){
    board[easyWin[0]][easyWin[1]] = currentPlayer;
    gameTurn += 1;
    displayBoard(board);
}
//But if he doesn't see a possible winning move, he checks several cases where he could win or tie
else{
